...
You can also use "save points" to rollback a transaction to a particular point in time if you don't want to rollback the entire transaction. This can be achieved through the use of Spring's link:{springapi}/org/springframework/transaction/SavepointManager.html[SavePointManager] interface.
