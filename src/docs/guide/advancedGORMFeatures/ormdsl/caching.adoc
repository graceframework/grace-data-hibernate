...
https://www.hibernate.org/orm/[Hibernate ORM] features a second-level cache with a customizable cache provider. This needs to be configured in the `app/conf/application.yml` file as follows:
...
NOTE: For further reading on caching and in particular Hibernate's second-level cache, refer to the link:{hibernateuserguide}/html_single/Hibernate_User_Guide.html#caching[Hibernate documentation] on the subject.
