...
The primary point of entry for using the Detached Criteria is the link:{gormapi}/grails/gorm/DetachedCriteria.html[DetachedCriteria] class which accepts a domain class as the only argument to its constructor:
...
Note that methods on the link:{gormapi}/grails/gorm/DetachedCriteria.html[DetachedCriteria] instance *do not* mutate the original object but instead return a new query. In other words, you have to use the return value of the `build` method to obtain the mutated criteria object:
...
The link:{gormapi}/grails/gorm/DetachedCriteria.html[DetachedCriteria] class can be used to execute batch operations such as batch updates and deletes. For example, the following query will update all people with the surname "Simpson" to have the surname "Bloggs":
