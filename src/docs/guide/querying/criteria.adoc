...
Criteria can be used either with the link:{gormapi}/org/grails/datastore/gorm/GormEntity.html#createCriteria()[createCriteria()] or link:{gormapi}/org/grails/datastore/gorm/GormEntity.html#withCriteria(groovy.lang.Closure)[withCriteria(closure)] methods.

The builder uses Hibernate's Criteria API. The nodes on this builder map the static methods found in the link:{hibernatejavadoc}/org/hibernate/criterion/Restrictions.html[Restrictions] class of the Hibernate Criteria API. For example:
...
Projections may be used to customise the results. Define a "projections" node within the criteria builder tree to use projections. There are equivalent methods within the projections node to the methods found in the Hibernate link:{hibernatejavadoc}/org/hibernate/criterion/Projections.html[Projections] class:
...
If the raw value or simple object array returned by the criteria method doesn't suit your needs, the result can be transformed with a link:{hibernatejavadoc}/org/hibernate/transform/ResultTransformer.html[ResultTransformer]. Let's say we want to transform the criteria results into a Map so that we can easily reference values by key:
...
We can also transform the result into an object of our choosing via the link:{hibernatejavadoc}/org/hibernate/transform/Transformers.html#aliasToBean-java.lang.Class-[Transformers.aliasToBean()] method. In this case, we'll transform it into an `AccountsOverview`:
...
You can use Hibernate's link:{hibernatejavadoc}/org/hibernate/ScrollableResults.html[ScrollableResults] feature by calling the scroll method:
...
If a node within the builder tree doesn't match a particular criterion it will attempt to set a property on the Criteria object itself. This allows full access to all the properties in this class. This example calls `setMaxResults` and `setFirstResult` on the link:{hibernatejavadoc}/org/hibernate/Criteria.html[Criteria] instance:
...
*listDistinct*,If subqueries or associations are used one may end up with the same row multiple times in the result set. This allows listing only distinct entities and is equivalent to `DISTINCT_ROOT_ENTITY` of the link:{hibernatejavadoc}/org/hibernate/criterion/CriteriaSpecification.html[CriteriaSpecification] class.
