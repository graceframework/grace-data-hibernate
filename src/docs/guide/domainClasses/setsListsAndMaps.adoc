...
If ordering and uniqueness aren't a concern (or if you manage these explicitly) then you can use the Hibernate link:{hibernateuserguide}/html_single/Hibernate_User_Guide.html#collections-bag[Bag] type to represent mapped collections.
...
[source,java]
----
def book = new Book(title: "New Grace Book")
