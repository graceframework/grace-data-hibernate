...
In order to activate schema-per-tenant multi-tenancy you need to set the multi-tenancy mode to `SCHEMA` in your configuration and supply a link:{gormapi}/org/grails/datastore/mapping/multitenancy/TenantResolver.html[TenantResolver]:
