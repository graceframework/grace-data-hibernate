...
The link:{gormapi}/org/grails/datastore/gorm/GormEntity.html#save()[save()] method will persist your class to the database using the underlying Hibernate ORM layer.

The `save()` method is defined by the link:{gormapi}/org/grails/datastore/gorm/GormEntity.html[GormEntity] trait.
...
This uses the static link:{gormapi}/org/grails/datastore/gorm/GormEntity.html#get(java.io.Serializable)[get(id)] method that expects a database identifier to read the `Person` object back from the database.
...
To delete an instance use the link:{gormapi}/org/grails/datastore/gorm/GormEntity.html#delete()[delete()] method:
